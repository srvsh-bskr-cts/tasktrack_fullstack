...
interface templateUpdateDate {
    stepId:any, newName: string
}

@Component({
  selector: 'app-change-workflow-step-name',
  standalone: true,
  ...
  <div class="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
    <div class="bg-zinc-900 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-w-lg w-full border border-zinc-800 pointer-events-auto transform transition-all animate-in overflow-hidden relative">
      ...
      <div class="px-8 py-6 border-b border-zinc-800/60 flex  justify-between items-center relative z-10">
        <div>
          <h2 class="text-xl font-black text-white uppercase tracking-tight">Rename Workflow Step</h2>
          <p class="text-[10px] font-bold text-zinc-500 captilize tracking-[0.2em] mt-1">
           This action will rename the step under workflow with id: {{workflowTemplateId}}, running workflows will remain unaffected
          </p>
        </div>
      ...
      @for (e of newWorkflowTemplateStepForm.stepId().errors(); track e) {
        <p class="text-[11px] font-bold text-red-400 uppercase tracking-wider flex items-center gap-2">
          ...
          {{ e.message }}
            ...
            <input type="text" 
                   [formField]="newWorkflowTemplateStepForm.newName"
            ...
            @if (this.newWorkflowTemplateStepForm.newName().invalid() && this.newWorkflowTemplateStepForm.newName().touched()) {
              <div class="space-y-1 mt-2 ml-1">
                @for (e of this.newWorkflowTemplateStepForm.newName().errors(); track e) {
                  <p class="text-[11px] font-bold text-red-400 uppercase tracking-wider flex items-center gap-2">
                    <span class="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]"></span>
                    {{ e.message }}
                  </p>
...
export class ChangeWorkflowTemplateStepNameDialog {
isOpen = model(false);
...
@Input() workflowTemplateSteps!:WorkflowStepTemplate[]
  ...
  workflowTemplateStepData= signal<templateUpdateDate>({
    stepId: null,
   newName: ''
  });
  newWorkflowTemplateStepForm = form(this.workflowTemplateStepData, (schemaPath) => {
    required(schemaPath.stepId, {message: "Step Id is required"}),
     required(schemaPath.newName, {message: "Name is required"})
   
   })

  private resetForm():templateUpdateDate{
    return {
          stepId: null,
newName: ''
  };
  ...
  onConfirm(): void {
    ...
    if(!this.newWorkflowTemplateStepForm().valid()) {
      ...
      this.workflowTemplateService.updateWorkflowStep(this.workflowTemplateId, this.workflowTemplateStepData().stepId!, {stepUpdate: [{operation: UpdateWorkflowStepOperationEnum.NAME,name: this.workflowTemplateStepData().newName}]})
      .subscribe(
        {
            next: () => {
                  this.isLoading.update(v => !v);
             ...
             this.cancel.emit(true)
                toast.success("Step Name Changed")
            ...
            error: () => {
                ...
                toast.error("Error while renaming step")
            }
        }
      )
